...
// doh! Bad ArbitraryCriterion includes
...
const QString PythonMatchCreator::POINT_POLYGON_SCRIPT_NAME = "PointPolygon.js";
...
public:
  ...
  PythonMatchVisitor(const ConstOsmMapPtr& map, vector<ConstMatchPtr>& result,
    ...
    if (!_index)
    {
      ...
      else
      {
        ...
        SpatialIndexer v(_index,
                         _indexToEid,
                         _pointPolyCrit,
                         std::bind(&PythonMatchCreator::getSearchRadius, this, placeholders::_1),
                         getMap());
        getMap()->visitWaysRo(v);
        getMap()->visitRelationsRo(v);
        v.finalizeIndex();
...
std::shared_ptr<PythonMatchCreator> PythonMatchCreator::_getCachedVisitor(
