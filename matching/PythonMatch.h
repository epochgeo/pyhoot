...
public:
  ...
  PythonMatch() = default;
  PythonMatch(const hoot::ElementId& eid1, const hoot::ElementId& eid2,
    const hoot::ConstMatchThresholdPtr& mt);
  ~PythonMatch() override = default;

  const hoot::MatchClassification& getClassification() const override { return _p; }
  ...
  void setMatchFunction(const std::function<int(int)> func) { _matchFunction = func; }
   ...
   */
  static hoot::MatchMembers geometryTypeToMatchMembers(const QString& geometryType);
...
private:
  ...
  bool _isWholeGroup;
  std::function<int(int)> _matchFunction;
  QString _matchName;
  bool _neverCausesConflict;
  hoot::MatchClassification _p;
  hoot::MatchMembers _matchMembers;
