...
PythonMatch::PythonMatch(const ElementId& eid1, const ElementId& eid2,
  const ConstMatchThresholdPtr& mt) :
Match(mt, eid1, eid2),
_isWholeGroup(false),
...
MatchMembers PythonMatch::geometryTypeToMatchMembers(const QString& geometryType)
{
  ...
  else if (geometryType == "unknown")
  {
    ...
    // note in PythonMatchVisitor::getIndex and rules/PointPolygon.js.
